...
const Pager = ({totItems, pageSize, currentPage, goToPage}) => {
    ...
    let clLink = (p) => {
        let cn;
        if (p === currentPage) cn = classNames([c.actLink]);
        else cn = classNames([c.pageLink]);
        return cn;
    };
    pages = pages.map(p => <span onClick={() => {goToPage(p)}}
                                 key={p} className={clLink(p)}>{p}</span>);
    if (firstPage>10) {
        let m20 = firstPage-10<1? 1 :firstPage-10;
        pages = [<span key={"-1"}><span onClick={() => {goToPage(m20)}}
    ...
    if (currentPage < totPages-10) {
        ...
        pages = [...pages, <span key={keymax}>&nbsp;<span onClick={() => {goToPage(p20)}}
